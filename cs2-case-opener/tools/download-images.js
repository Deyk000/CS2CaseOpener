...
function buildSourceSteps(entry) {
  ...
  // Prefer community-hosted previews (CSFloat, CSGOStash) before Steam Market
  // because Steam Market often returns generic preview cards instead of raw skin art.
  return [
...
async function main() {
  ...
  await writeFile(path.join(REPORTS_DIR, 'image-map.json'), JSON.stringify(imageMap, null, 2), 'utf8');
  ...
  ensureDirectoryExists(REPORTS_DIR);
  await writeFile(path.join(REPORTS_DIR, 'download-report.txt'), report, 'utf8');
  await writeFile(path.join(REPORTS_DIR, 'missing-images.json'), JSON.stringify(failures, null, 2), 'utf8');

  if (failures.length > 0) {
    await writeFile(path.join(REPORTS_DIR, 'failed-images.html'), buildFailedHtml(failures), 'utf8');
  }
