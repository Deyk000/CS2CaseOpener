...
async function main() {
  ...
  const reportPath = path.join(reportsDir, 'download-report.txt');
  ...
  console.log(`📄 Report saved to: reports/download-report.txt\n`);

  // Save failed images list as JSON
  const missingPath = path.join(reportsDir, 'missing-images.json');
  ...
  console.log(`📄 Missing images JSON saved to: reports/missing-images.json\n`);

  // Generate fallback HTML
  if (stats.failures.length > 0) {
    const htmlPath = path.join(reportsDir, 'failed-images.html');
...
<body>
  ...
  <div class="item">
    ...
    console.log(`📄 Fallback HTML saved to: reports/failed-images.html\n`);
  }
